...
def calculate_recovery_time_ms(gateway: ReliabilityGateway) -> float | None:
    ...
    for breaker in gateway.breakers.values():
        ...
        for transition in breaker.transition_log:
            ...
            elif transition.get("to") == "closed" and opened_ts is not None:
                closed_ts = float(transition["ts"])
                recovery_times_ms.append((closed_ts - opened_ts) * 1000)
                opened_ts = None
...
def _scenario_passed(name: str, metrics: RunMetrics) -> bool:
    ...
    if name == "primary_flaky_50":
        return metrics.availability >= 0.95 and metrics.circuit_open_count >= 1
    if name == "all_healthy":
